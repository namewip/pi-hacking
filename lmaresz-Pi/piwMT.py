...
def calculate(it, prec, fb, rf):
    ...
    t = Thread(
        ...
        args=(
            ...
            fb,
            rf,
        ))  # itt lehet változtatni a 2-3. argumentumot
    t.start()
...
def progression(osszes, felbontas=20, refresh=0.25):
    while True:
        ...
        if n == it:  # ha kész van a művelet
            os.system('clear')
            # újra kiszámít mindent és kiirat egy törlés után
            prog = (n / (osszes - 1))
            print("{:.2%}".format(prog), end=" ")
            print(progressbar(n, osszes - 1, felbontas))
            print(
                "Pontosság:{} tizedesjegy".format(
                    compare.test()))  # megadja a pontosságot
            print(
                "A PI értéke sikeresen kiszámításra került, megtekinthető az out.txt fájlban")
            break
        sleep(refresh)


# elkészíti a kis ábrát a progression barhoz
def progressbar(pill, ossz, felbontas):
    # rész/egész*100 -> százalék érték -> 0<ertek<100
    ertek = round(pill / ossz, 2) * 100
